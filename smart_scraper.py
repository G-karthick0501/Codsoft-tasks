...
# ─── GitHub API session (with token rotation support) ────────────────────────
class GitHubSession:
    ...
    def __init__(self, tokens: list[str]):
        ...
        log.info(f"GitHubSession ready with {len(tokens)} token(s) rotating")

    def _next_session(self) -> tuple[str, requests.Session]:
        with self._lock:
            token = next(self._token_cycle)
        return token, self._sessions[token]

    def __init__old(self, token: str):  # kept for reference, unused
    ...
    def get(self, url: str, **kwargs) -> requests.Response:
        """GET with automatic rate-limit back-off, rotating tokens on 403."""
        time.sleep(0.05)  # polite delay
        for attempt in range(5):
            token, session = self._next_session()
            ...
            if r.status_code == 403:
                ...
                log.warning(f"Rate limit hit (remaining={remaining}) — rotating token, sleeping {min(wait,10):.0f}s")
                time.sleep(min(wait, 10))  # max 10s wait — other token will take over
                continue
    ...
    def get_file_tree(self, repo: str, sha: str = "HEAD") -> list[dict]:
        ...
        if data.get("truncated"):
            log.warning(f"{repo}: tree truncated — partial results only")
    ...
    def get_file_content(self, repo: str, path: str, branch: str = "main") -> Optional[str]:
        """CDN download — no rate limit for public repos."""
...
def scrape_repo(repo: str, gh: GitHubSession, max_files: int = 0) -> list[dict]:
    ...
    # CDN downloads — 4 threads to stay within 1GB RAM budget
...
def main():
    ...
    parser.add_argument(
        ...
        help="Primary GitHub token (or set GITHUB_TOKEN env var)"
    )
    parser.add_argument(
        "--token2",
        default=os.environ.get("GITHUB_TOKEN_2", ""),
        help="Second GitHub token for rotation (doubles rate limit)"
    )
    parser.add_argument(
        "--output", "-o",
        default="master2.csv",
    ...
    # ── Token setup ───────────────────────────────────────────────────────────
    tokens = [t for t in [args.token, args.token2] if t]
    if not tokens:
        # Try loading from .env directly
        tokens = [v for k, v in os.environ.items() if k.startswith('GITHUB_TOKEN') and v]
    if not tokens:
        print("ERROR: GitHub token required. Pass --token, set GITHUB_TOKEN, or add to .env")
        sys.exit(1)
    log.info(f"Using {len(tokens)} GitHub token(s) with rotation")
